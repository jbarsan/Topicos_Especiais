...
def game_detail(request, pk):
    ...
    if request.method == 'GET':
        ...
        # Pegando a data de hoje e inserindo o timezone local,
        # se não fizer isso gera uma exceção chata.
        hoje = timezone.make_aware(datetime.now(), timezone.get_current_timezone())
        # Comparando as datas de hoje com a data de lançamento do jogo
        # Se for True retorna o status code 412 (Falha de pré-condição)
        if hoje >= game.release_date:
            return Response({'release_date': 'O jogo já foi lançado.'}, status=status.HTTP_412_PRECONDITION_FAILED)
        else:
            game.delete()
